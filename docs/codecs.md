...
| AMR-NB | `AMR` (dynamic) | all 8 modes | MR475 + MR122 only | 3GPP TS 26.074 vectors | `amr` |
...
bit-exact against 3GPP TS 26.074. Encode is wired for MR475 (4.75 kbit/s) and MR122
(12.2 kbit/s, the GSM-EFR-equivalent mode) only; a `mode-set` that forces one of the
other six modes is declined cleanly at call setup rather than mis-encoding mid-call.
That is enough for AMR-NB to G.711 transcoding in both directions; the remaining
encoder modes are in progress.
