...
|---|---|---|
| `transport_protocol` | string | Far-leg transport, e.g. `RTP/AVP`, `RTP/SAVP` (SDES-SRTP, RFC 4568), `UDP/TLS/RTP/SAVPF` (DTLS-SRTP, RFC 5764). Naming `RTP/SAVP` on an `offer` whose caller is *also* SDES-SRTP anchors a **transcrypt**: the engine advertises its own key to each party and re-encrypts every datagram from one party's key to the other's, so neither key reaches the other side. By default the payload is not decoded, so any codec crosses; ask for the audio (`record_call`, `noise_suppression`, `echo_cancellation`, `beep_detection`) or let the two parties answer different codecs, and the same pair goes through the media actor with both legs keyed instead — about 890 ns per packet against 510 ns for the bridge. `checkpoint` is refused for either shape (the snapshot record holds one secure leg, the call has two); the call itself is unaffected. Naming a **DTLS** far leg for an SDES caller is refused on the `offer` itself — two keying mechanisms rather than two keys — so no call is created and the callee is never rung. |
