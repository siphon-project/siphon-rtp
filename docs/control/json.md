...
| `ws_bridge_ended` | `call_id`, `from_tag`, `stream_id`, `reason` | The takeover bridge stopped. `reason` is `detached` (a detach or a re-point — the only orderly end), `server_closed`, `server_stopped`, `call_ended` or `transport_error`. Emitted exactly once per started bridge. **Anything but `detached` means a live call has no far side**: a takeover bridge is leg A's only peer, so unlike a dead tee this is silence on the call, not a lost copy of it. Re-point the bridge, detach it, or tear the call down. |
