...
|---|---|---|
| `transport_protocol` | string | Far-leg transport, e.g. `RTP/AVP`, `RTP/SAVP` (SDES-SRTP, RFC 4568), `UDP/TLS/RTP/SAVPF` (DTLS-SRTP, RFC 5764). Naming a **secure** far leg on an `offer` whose caller is itself secure is refused there and then (`secure-offerer-unsupported`), because bridging two secure parties needs a transcrypt between two different keys: the `offer` returns the error and no call is created, so the callee is never rung for a topology the engine cannot carry. A secure caller toward a plain (`RTP/AVP`) callee is supported. |
