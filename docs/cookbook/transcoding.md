...
| AMR-NB (TS 26.071/.090) | `amr` | all 8 modes | **MR475 and MR122 only** | bit-exact, 3GPP TS 26.074 vectors |

Be precise about AMR-NB: the decoder covers every mode, the encoder covers two. The engine's
default AMR-NB egress mode is MR122 (12.2 kbit/s, the GSM-EFR-compatible mode); MR475 is
implemented and bit-exact but the SDP layer does not currently select it. AMR-WB egress defaults
