...
//! recipe in CONTRIBUTING.md): `tests/silk_header_conformance.rs` for §4.2.3-§4.2.7.4 and
//! `tests/silk_nlsf_conformance.rs` for §4.2.7.5 — the latter diffs the coded indices, the
//! dequantized residual with its unpacked prediction weights and entropy-table selection, the
//! reconstruction *before* and *after* stabilisation, the interpolated vector, and both Q12 LPC
//! coefficient sets.
//!
//! Extending that for a new sub-phase is two steps: add the field group to
//! `reference/opus/silk_trace.patch` (it is one shared patch — extend it, never replace it) and
//! decode one more symbol group per packet in a new harness before it stops. Treat these as the
//! working diagnostic and the two whole-packet gates as the acceptance criterion — an
//! intermediate-state diff proves the fields match, not that the packet parses to its end.
//!
