...
/// RFC 4867 bit-reorder table for AMR-WB 6.60 kbit/s (132 bits) (`sort_660`).
#[rustfmt::skip]
pub static SORT_660: [u16; 132] = [
...
/// RFC 4867 bit-reorder table for AMR-WB 8.85 kbit/s (177 bits) (`sort_885`).
#[rustfmt::skip]
pub static SORT_885: [u16; 177] = [
...
/// RFC 4867 bit-reorder table for AMR-WB 12.65 kbit/s (253 bits) (`sort_1265`).
#[rustfmt::skip]
pub static SORT_1265: [u16; 253] = [
...
/// RFC 4867 bit-reorder table for AMR-WB 14.25 kbit/s (285 bits) (`sort_1425`).
#[rustfmt::skip]
pub static SORT_1425: [u16; 285] = [
...
/// RFC 4867 bit-reorder table for AMR-WB 15.85 kbit/s (317 bits) (`sort_1585`).
#[rustfmt::skip]
pub static SORT_1585: [u16; 317] = [
...
/// RFC 4867 bit-reorder table for AMR-WB 18.25 kbit/s (365 bits) (`sort_1825`).
#[rustfmt::skip]
pub static SORT_1825: [u16; 365] = [
...
/// RFC 4867 bit-reorder table for AMR-WB 19.85 kbit/s (397 bits) (`sort_1985`).
#[rustfmt::skip]
pub static SORT_1985: [u16; 397] = [
...
/// RFC 4867 bit-reorder table for AMR-WB 23.05 kbit/s (461 bits) (`sort_2305`).
#[rustfmt::skip]
pub static SORT_2305: [u16; 461] = [
...
/// RFC 4867 bit-reorder table for AMR-WB 23.85 kbit/s (477 bits) (`sort_2385`).
#[rustfmt::skip]
pub static SORT_2385: [u16; 477] = [
...
/// Speech-bit counts per AMR-WB mode (0..=8), RFC 4867 Table 1a (`unpacked_size`).
pub const NB_BITS: [usize; 9] = [132, 177, 253, 285, 317, 365, 397, 461, 477];

/// The RFC 4867 bit-reorder (sort) table for AMR-WB speech `mode` (0..=8). `sort_table(m)[i]` is the
/// encoder-order index of the `i`-th RTP-payload bit (`sort_ptr[mode]`, `mime_io.tab`).
#[must_use]
pub fn sort_table(mode: u8) -> &'static [u16] {
    match mode {
        0 => &SORT_660,
        1 => &SORT_885,
        2 => &SORT_1265,
...
/// Un-sort an RTP-payload speech frame (MSB-first bytes in `data`) into encoder-order
/// `BIT_0`/`BIT_1` words for the core decoder of `mode` (0..=8), using the RFC 4867 sort table
/// (`Read_serial` MIME path). The returned vector has `NB_BITS[mode]` entries; missing payload
/// bits read as `BIT_0` so a truncated frame never panics (the caller validates length first).
#[must_use]
pub fn unsort(data: &[u8], mode: u8) -> Vec<i16> {
    let sort = sort_table(mode);
    let mut prms = vec![BIT_0; sort.len()];
    for (i, &dest) in sort.iter().enumerate() {
...
mod tests {
    ...
    #[test]
    fn every_sort_table_is_a_permutation_of_its_range() {
        // All 9 RFC 4867 sort tables must be bijections over 0..NB_BITS[mode].
        for mode in 0u8..=8 {
            let sort = sort_table(mode);
            assert_eq!(sort.len(), NB_BITS[mode as usize], "mode {mode} length");
            let mut seen = vec![false; sort.len()];
            for &d in sort {
                assert!(!seen[d as usize], "mode {mode} duplicate index {d}");
                seen[d as usize] = true;
            }
            assert!(seen.iter().all(|&s| s), "mode {mode} covers every index");
        }
    }

    #[test]
    fn generic_unsort_inverts_each_modes_payload_sort() {
        for mode in 0u8..=8 {
            ...
            let enc: Vec<i16> = (0..nbits)
                ...
                .collect();
            let sort = sort_table(mode);
            let mut payload = vec![0u8; nbits.div_ceil(8)];
